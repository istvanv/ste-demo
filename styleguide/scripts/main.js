/* PrismJS 1.16.0
 https://prismjs.com/download.html#themes=prism-dark&languages=markup+css+clike+javascript+scss+pug */
...
(function($) {
    ...
    // Toggle element visibility (i.e. code preview) - slide up/down

    let $sg_component = $('.js-sg-showmore');

    $sg_component.each(function() {
        let $sg_showmoreTrigger = $('.js-sg-showmore__trigger', this),
            $sg_showmoreTarget = $('.js-sg-showmore__target', this);

        // Apply initial state
        $sg_showmoreTarget.hide();

        // Attach interaction behaviour
        $sg_showmoreTrigger.on('click', function() {
            $sg_showmoreTarget.slideToggle().toggleClass('is-active');
            $sg_showmoreTrigger.toggleClass('is-active');
        });
    });



    // Load page in iframe from URL hash

    let targetFrame = $('#sg-application-pages');
    ...
    // Toggle default nav visibility based to screen resolution
    // if enough space, just have it open.

    let $screenWrapper = $('.sg-layout'),
        $navToggle = $('.js-sg-toggle-nav');
    ...
    function toggleNav(state) {
        if (state === 'on') {
            if (!$screenWrapper.hasClass('is-nav-open')) {
                $screenWrapper.addClass('is-nav-open');
                $navToggle.addClass('is-active');
            }
        } else {
            $screenWrapper.removeClass('is-nav-open');
            $navToggle.removeClass('is-active');
        }
    }


    // Control sidebar visibility with toggle button
    $navToggle.on('click', function() {
        $screenWrapper.toggleClass('is-nav-open');
