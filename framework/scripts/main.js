/**
 * Manually added and custom scripts only.
 * Vendor (/node_modules) stuff is included in /vendor/bundle.js and /vendor/bundle.min.js
 *
 * The script files listed here are included and merged by the gulpfile
 * using gulp-include see https://www.npmjs.com/package/gulp-include
 */
...
/** ========================================================
 * Components - third-party includes
 */
...
;(function($) {
	...
	$(document).ready(function(){
		...
		const $owlInstances = $('.js-owl-carousel');

		const updateCount = function(e, $customCount) {
			let count = (e.page.index === 0) ? 1 : (e.page.index + 1);
			$customCount.text(count + ' of ' + e.item.count);
		};
		...
		$owlInstances.each(function () {
			...
			// Create this carousel instance
			$owlInstance.owlCarousel({
				loop: true,
				items: 1,
				nav: owlShowControls,
				dotsSpeed: 800,
			...
			// If a 'slide count' display element was specified
			if ($owlCustomCount.is('*')) {
				$owlInstance.on('changed.owl.carousel', function (e) {
					updateCount(e, $owlCustomCount);
				});
			}
		});
...
Via onClick (for dynamic content compatibility):

<a href="#"
   onclick="openModalSm(event)"
   data-modal-target="#modal_modal-content-container">Trigger</a>


OR for automatic event listener attachment:


<button type="button"
        class="js-modal-trigger"
        data-modal-target="#modal_modal-content-container">Trigger</button>


Optional attributes to go on the modal content div:

data-modal-save-label="Custom save"
data-modal-cancel-label="Custom cancel"
*/
...
;(function($) {
    ...
    $(document).ready(function(){
        const $otElements = $('.js-overlay-tabs');

        $otElements.each(function () {
            const $otThis = $(this);
            const $otItems = $('.js-overlay-tabs__item', $otThis);

            // Depending on breakpoint, open the first tab item
            if (breakPoint.check.mdUp()) {
                $otItems.first().addClass('c-overlay-tabs__item--current');
            }

            $otItems.each(function () {
                const $otItem = $(this);
                const $otTrigger = $('.js-overlay-tabs__trigger', $otItem);
                const $otTarget = $('.js-overlay-tabs__target', $otItem);

                $otTrigger.on('click', function (e) {
                    e.stopPropagation();

                    // On mobile only, we want tapping on an active tab to toggle
                    // that tab's status. Otherwise we do nothing.
                    if ($otItem.hasClass('c-overlay-tabs__item--current')) {
                        if (breakPoint.check.smDown()) {
                            $otItem.removeClass('c-overlay-tabs__item--current');
                        }
                    ...
                    // If a non-active item was clicked, set this item to be active, reset all others
                    // Everything apart from auto-scrolling is done in CSS
                    $otItems.removeClass('c-overlay-tabs__item--current');
                    $otItem.addClass('c-overlay-tabs__item--current');
                    ...
                    setTimeout(function () {
                        if (breakPoint.check.smDown()) {
                            ...
                            $('html, body').animate({
                                scrollTop: $otTrigger.offset().top
                            }, 600);
                        } else {
                            // On desktop, scroll to make sure the top of the tab content is visible
                            $('html, body').animate({
                                scrollTop: ($otTarget.offset().top - 16)
                            }, 600);
                        }
                    }, 100);
                });
...
;(function ($) {
    ...
    // Capture clicks outside of dropdownify elements, and close any that are open
...
const breakPoint = (function() {
    ...
    const check = {
        xsDown : function() { return window.matchMedia('(max-width: 699px)').matches; },
        smDown : function() { return window.matchMedia('(max-width: 999px)').matches; },
        mdDown : function() { return window.matchMedia('(max-width: 1399px)').matches; },
        lgDown : function() { return window.matchMedia('(max-width: 1999px)').matches; },

        xsUp : function() { return window.matchMedia('(min-width: 700px)').matches; },
        smUp : function() { return window.matchMedia('(min-width: 1000px)').matches; },
        mdUp : function() { return window.matchMedia('(min-width: 1400px)').matches; },
        lgUp : function() { return window.matchMedia('(min-width: 2000px)').matches; }
    };
